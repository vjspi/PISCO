...
class NIKSirenPatch(NIKSoSBase):
    ...
    def create_optimizer(self):
        ...
        # self.optimizer = torch.optim.Adam([self.parameters(), self.network.parameters()], lr=self.config['lr'])
        if "amsgrad" in config_optim and config_optim["amsgrad"] == True:
            config_optim["amsgrad"] = False
            self.config["optimizer"]["params"]["amsgrad"] = False
            print("Warning: Amsgrad not possible for convex convolution training, DISABLING!")
    ...
    def pre_process(self, inputs,  conv = True):
        ...
        if conv:
            ...
            coords = inputs['coords_patch'].to(self.device)  # required for loss calculation
            inputs['coords'] = inputs['coords'].to(self.device)  # required for loss calculation

        else:
            inputs['coords'] = inputs['coords'].to(self.device)  # required for loss calculation#
            coords = inputs['coords'].to(self.device)
        ...
        if "encoding" in self.config:
            if self.config["encoding"]["type"] == "spatial":
                ...
                features = torch.cat([torch.cos(factor * coords[:, 1:] @ self.network_kdata.B_spat),
                                      ...
                                      torch.cos(factor * coords[:, 1:] @ self.network_kdata.B_temp) * torch.cos(
                                          factor * coords[:, :1]),
                                      torch.cos(factor * coords[:, 1:] @ self.network_kdata.B_temp) * torch.sin(
                                          factor * coords[:, :1]),
                                      torch.sin(factor * coords[:, 1:] @ self.network_kdata.B_temp) * torch.cos(
                                          factor * coords[:, :1]),
                                      torch.sin(factor * coords[:, 1:] @ self.network_kdata.B_temp) * torch.sin(
                                          factor * coords[:, :1])
                                      ], dim=-1)
                ...
                features = torch.cat([torch.cos(factor * coords[:, 1:] @ self.network_kdata.B_spat),
                                      torch.sin(factor * coords[:, 1:] @ self.network_kdata.B_spat),
                                      torch.cos(factor * coords[:, 1:] @ self.network_kdata.B_dyn) * torch.cos(
                                          factor * coords[:, :1]),
                                      torch.cos(factor * coords[:, 1:] @ self.network_kdata.B_dyn) * torch.sin(
                                          factor * coords[:, :1]),
                                      torch.sin(factor * coords[:, 1:] @ self.network_kdata.B_dyn) * torch.cos(
                                          factor * coords[:, :1]),
                                      torch.sin(factor * coords[:, 1:] @ self.network_kdata.B_dyn) * torch.sin(
                                          factor * coords[:, :1]),
                                      torch.sin(factor * coords[:, :1] @ self.network_kdata.B_temp),
                                      torch.cos(factor * coords[:, :1] @ self.network_kdata.B_temp),
                                      ], dim=-1)
    ...
    def test_batch(self, input=None, conv = True):
        ...
        with torch.no_grad():
            ...
            if input is None:
                ...
                kpred = kpred.reshape(input.shape[0], input.shape[1], input.shape[2], -1).permute(0,3,1,2) # coil dimension second, imgDim last
                k_outer = 1
                kpred[dist_to_center >= k_outer] = 0
