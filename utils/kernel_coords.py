...
def create_cartesian_kernel(coords, kernel_size, patch_dist, delta_dist, middle_index=False):
    ...
    coord_neighbors[..., -2:] = coord_neighbors[..., -2:] + kernel_flat  ## adding kernel offset here
    return coord_neighbors

def create_cartesion_kernel_XY_indices(kernel_size, patch_dist, exclude_center="x"):
    dpatch = [int(np.floor(k / 2)) for k in kernel_size]
    kernel = np.stack(np.meshgrid(np.linspace(-dpatch[0], dpatch[0], 2 * dpatch[0] + 1),
                                        np.linspace(-dpatch[1], dpatch[1], 2 * dpatch[1] + 1),
                                        indexing="ij"), axis=-1)
    ...
    if exclude_center == "x":
        middle_index = kernel.shape[0] // 2
        kernel = np.concatenate((kernel[:middle_index, ...], kernel[middle_index + 1:,...]), axis=0)
    elif exclude_center == "y":
        middle_index = kernel.shape[1] // 2
        kernel = np.concatenate((kernel[:, :middle_index, ...], kernel[:, middle_index + 1:,...]), axis=1)
    return kernel

def create_cartesian_kernel_XY(coords, kernel_size, patch_dist, delta_dist, exclude_center="x"):

    if coords.ndim != 2:
        coords = coords.reshape(-1, coords.shape[-1])

    kernel = create_cartesion_kernel_XY_indices(kernel_size, patch_dist, exclude_center=exclude_center)
