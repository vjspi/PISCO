...
def create_hystereses(img, dim_axis, final_dim=None):
    img = torch2numpy(img)
    final_dim = final_dim if final_dim is not None else 100
...
def postprocess(pred, ref = None):
    ...
    pred = utils.mri.clip_to_percentiles(pred, 0.00, 0.99)
...
def log_difference_images(pred_img, ref, mag_factor=1):
    ...
    pred_corr = pred
    # pred = np.zeros_like(pred)
    # pred_corr = medutils.visualization.normalize(pred, vmax=1.)
    # pred_corr = np.stack([bias_corr(pred[i, ...], ref[i, ...], mag=False)
    #                       for i in range(pred.shape[0])])  # loop over dynamics
...
def log_recon_to_wandb(kpred_all, csm, ref=None, multi_coil=True, log_xt=True, log_diff=False, cycle_duration=5):
    log_results = {}
    ...
    fps = np.floor((vis_img['k_mag_coil0'].shape[0]) / cycle_duration)
    ...
    for ech in range(kpred_all.shape[1]):
        ...
        log_results.update({
            ...
            'img' + str(ech): wandb_log_type_func(vis_img['combined_mag'][:,ech,...]),
        ...
        if log_xt:
            # ToDo: expand
            idx = vis_img["combined_mag"].shape[-1] // 2 + 2
            # img_marked = vis_img["combined_mag"].copy()
            # img_marked[:,ech, :, :, int(idx)] = np.max(img_marked)
            log_results.update({
                # 'img_t_v': wandb.Image(img_marked[0, ech, 0, :, :]),
                'img_t-centerY': wandb.Image((vis_img["combined_mag"][:, ech, 0, :, int(idx)])),
                'img_t_phase-centerY': wandb.Image((vis_img["combined_phase"][:, ech, 0, :, int(idx)]))
            })

            idx = vis_img["combined_mag"].shape[-2] // 2 + 2
            # img_marked = vis_img["combined_mag"].copy()
            # img_marked[:,ech, :, :, int(idx)] = np.max(img_marked)
            log_results.update({
                # 'img_t_v': wandb.Image(img_marked[0, ech, 0, :, :]),
                'img_t-centerX': wandb.Image((vis_img["combined_mag"][:, ech, 0, int(idx), :])),
                'img_t_phase-centerX': wandb.Image((vis_img["combined_phase"][:, ech, 0, int(idx), :]))
            })

        if log_diff:
            log_results.update({
                'img_t_v': wandb.Image(img_marked[0, ech, 0, :, :]),
                'img_t': wandb.Image((vis_img["combined_mag"][:, ech, 0, :, int(idx)]))
            })
...
import lpips
from torchvision.models import AlexNet_Weights, VGG16_Weights  # Import relevant weight enums


def get_lpips_model(net, weights=None):
    """Singleton pattern to get the LPIPS model with enum-based weight initialization."""

    if net == 'alex':
        weights = AlexNet_Weights.DEFAULT  # Use the default AlexNet weights
    elif net == 'vgg':
        weights = VGG16_Weights.DEFAULT  # Use the default VGG weights
    else:
        raise ValueError(f"Unsupported network type: {net}")

    # Check if model exists or needs to be reinitialized
    if not hasattr(get_lpips_model, "model") or get_lpips_model.net != net or get_lpips_model.weights != weights:
        # Initialize LPIPS model with the given net and proper weights
        get_lpips_model.model = lpips.LPIPS(net=net, pretrained=True)
        get_lpips_model.net = net  # Store net type
        get_lpips_model.weights = weights  # Store weights

    return get_lpips_model.model
...
def get_eval_metrics(img, ref, axes=(1,2), mean=True):
    ...
    """
    # global lpips_model  # Access the global lpips_model variable

    if img.dtype != ref.dtype:
        # ref = ref.astype(img.dtype)
        img = img.astype(ref.dtype)

    metrics_dict = {
        "ssim": [ssim(img[i], ref[i], axes=axes) for i in range(img.shape[0])],
        "psnr": [psnr(img[i], ref[i], axes=axes, max_intensity=1.0) for i in range(img.shape[0])],
        ...
        "fsim": [fsim(img[i].squeeze()[...,None] * 255.0, ref[i].squeeze()[...,None] * 255.0) for i in range(img.shape[0])],
    }

    # Learned Perceptual Image Patch Similarity (LPIPS) metric
    lpips_model_alex = get_lpips_model("alex")
    metrics_dict["lpips_alex"] = [lpips_model_alex.forward(numpy2torch(medutils.visualization.normalize(img[i], vmin=-1, vmax=1, max_int=1)),
                                                 numpy2torch(medutils.visualization.normalize(ref[i], vmin=-1, vmax=1, max_int=1))).item() for i in range(img.shape[0])]
    lpips_model_vgg = get_lpips_model("vgg")
    metrics_dict["lpips_vgg"] = [lpips_model_vgg.forward(numpy2torch(medutils.visualization.normalize(img[i], vmin=-1, vmax=1, max_int=1)),
                                                 numpy2torch(medutils.visualization.normalize(ref[i], vmin=-1, vmax=1, max_int=1))).item() for i in range(img.shape[0])]

    # Temporal FSIM
    # phantom data limits : [70:155, 60:165]
    if img.shape[0] > 1:
        metrics_dict["fsim_xt"] = [fsim(img[:,:,i,:].squeeze()[...,None] * 255.0, ref[:,:,i,:].squeeze()[...,None] * 255.0) for i in range(img.shape[-2])]
        metrics_dict["fsim_yt"] = [fsim(img[:,:,:,i].squeeze()[...,None] * 255.0, ref[:,:,:,i].squeeze()[...,None] * 255.0) for i in range(img.shape[-2])]
    if img.shape[0] > 1:
        # Apply padding to the first dimension (temporal) to enable lpips calculation
        pad_size = (32 - img.shape[0]) // 2 if img.shape[0] < 32 else 0

        img_padded = np.pad(img, ((pad_size, pad_size), (0, 0), (0, 0), (0, 0)), mode='constant', constant_values=0)
        ref_padded = np.pad(ref, ((pad_size, pad_size), (0, 0), (0, 0), (0, 0)), mode='constant', constant_values=0)

        metrics_dict["lpips_xt_alex"] = [lpips_model_alex.forward(numpy2torch(medutils.visualization.normalize(img_padded[:,:,i,:].transpose(1,0,2), vmin=-1, vmax=1, max_int=1)),
                                                 numpy2torch(medutils.visualization.normalize(ref_padded[:,:,i,:].transpose(1,0,2), vmin=-1, vmax=1, max_int=1))).item() for i in range(img_padded.shape[-2])]
        metrics_dict["lpips_yt_alex"] = [lpips_model_alex.forward(numpy2torch(medutils.visualization.normalize(img_padded[:,:,:,i].transpose(1,0,2), vmin=-1, vmax=1, max_int=1)),
                                                 numpy2torch(medutils.visualization.normalize(ref_padded[:,:,:,i].transpose(1,0,2), vmin=-1, vmax=1, max_int=1))).item() for i in range(img_padded.shape[-2])]

        metrics_dict["lpips_xt_vgg"] = [lpips_model_vgg.forward(numpy2torch(medutils.visualization.normalize(img_padded[:,:,i,:].transpose(1,0,2), vmin=-1, vmax=1, max_int=1)),
                                                 numpy2torch(medutils.visualization.normalize(ref_padded[:,:,i,:].transpose(1,0,2), vmin=-1, vmax=1, max_int=1))).item() for i in range(img_padded.shape[-2])]
        metrics_dict["lpips_yt_vgg"] = [lpips_model_vgg.forward(numpy2torch(medutils.visualization.normalize(img_padded[:,:,:,i].transpose(1,0,2), vmin=-1, vmax=1, max_int=1)),
                                                 numpy2torch(medutils.visualization.normalize(ref_padded[:,:,:,i].transpose(1,0,2), vmin=-1, vmax=1, max_int=1))).item() for i in range(img_padded.shape[-2])]
